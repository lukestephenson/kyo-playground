...
//
//trait MyService {
//  def hello(name: String): Unit < Console
//}
//
//trait Subscriber[A] {
//  def onNext(e: A): Unit
...
//
//object Observable {
//  def range(start: Int, end: Int): Observable[Int] < IO = new Observable[Int] {
...
//    val channel: Channel[String] < IO = Channels.init[String](10)
//    val program = for {
//      name <- maybeName
//      _ <- Console.println(s"Name is $name")
//      service <- Envs.get[MyService]
//      _ <- service.hello(name)
//      _ <- Console.println(s"Add to channel")
//      liveChannel <- channel
//      _ <- liveChannel.put(name)
//      _ <- Console.println(s"Take from channel")
//      taken <- liveChannel.take
//      _ <- Console.println(s"from a channel took $taken")
//    } yield ()
//
...
//    }
//
//    val streamResult: (Long, Unit) < (Fibers & Console) = for {
//      start <- Clocks.now
//      tuple <- stream.transform(transformFn).runFold(0L)(_+_)
//      end <- Clocks.now
//      _ <- Console.println(s"Took ${end.toEpochMilli()-start.toEpochMilli()}ms to calculate $tuple")
//    } yield tuple
//
////    val program2: Unit < (Options & IO & Envs[MyService]) = maybeName.flatMap { name =>
////      for {
////        _ <- Console.println(s"Name is $name")
...
////    }
////    for {
////      _ <- maybeName.flatMap(name => Console.println(s"Name is ${maybeName}"))
////      _ <- Console.println(s"Name is ${maybeName}")
////    } yield ()
//    val exampleService = new MyService {
//      override def hello(name: String): Unit < Console = Console.println(s"Hello $name $streamResult")
//    }
